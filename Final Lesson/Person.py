...
class Person:
    ...
    def do(self, action: Action):
        ...
        if type(action == Rest):
            if(self.health < 40):
                self.change_state(action.health, action.mood - ((action.mood / 100) * 20), action.money)
            else:
                self.change_state(action.health, action.mood, action.money)


people = [Person, Person, Person]
people[0] = Person("Iarik", 100, 100, 80)
people[1] = Person("Petya", 200, 50, 100000)
people[2] = Person("Ahmed", 300, 200, 20)
actions = [Action, Action, Action]
actions[0] = Rest("Park", 10, 20, -10)
actions[1] = Work("Factory", -40, -30, 50)
actions[2] = Action("Shop", 30, 30, -50)
while True:
    try:
